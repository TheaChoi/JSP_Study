...
public class UserDAO 
{
   public ArrayList<ListMainDTOOut> listmain(String type,int cnt) throws SQLException
   {
      ...
      while(rs.next()==true){
    	  ...
    	  dto = new ListMainDTOOut(num, type, title, photo, content);
...
//    	  System.out.println(photo);
   ...
   public ArrayList<UserDTOOut> list(pageDTOIn page) throws SQLException{
		...
		int mode = page.getMode(); //-1검색어 미사용, 0이면 검색어 사용 리스트 만든다
	 	...
	 	System.out.println("넘버:"+pageNo+pageSize+"모드:"+mode+type);
		...
		if(mode!=0)
		{
			String sql = "select num, title, reg_date, count, photo, content from board where type=? order by num desc limit ?,?";
   ...
   public int countAll(String type, pageDTOIn page) throws SQLException{
		...
		System.out.println("page mode는 "+page.getMode());
