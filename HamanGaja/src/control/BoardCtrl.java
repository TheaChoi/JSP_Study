...
public class BoardCtrl extends HttpServlet {
    ...
    public void list(HttpServletRequest request, HttpServletResponse response) throws SQLException, ServletException, IOException {
    	...
    	int pageNo = Integer.parseInt(request.getParameter("pageNo"));  //현재페이지 번호(0부터 시작)
    	int pageSize = Integer.parseInt(request.getParameter("pageSize")); //노출 페이지 개수
    	int viewStart =  Integer.parseInt(request.getParameter("viewStart")); //노출되는 첫 페이지 번호
    	int viewEnd =  Integer.parseInt(request.getParameter("viewEnd")); //노출되는 마지막 페이지 번호
    	
    	String type = request.getParameter("type");
    	type = URLDecoder.decode(type, "UTF-8") ;  //get파라미터에 한글이 깨질때
    	
    	int mode = 0;  //검색어가 있을 경우 mode=0
    				//검색어
